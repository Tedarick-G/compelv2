...
async function collectLinks(brand) {
  ...
  while(true) {
    ...
    try {
      const res = await workerGet({
        u: brand,
        page,
        fields: 'urun_linki'
      });

      const links = (res.urunler||[])
        .map(u=>u.urun_linki)
        .filter(Boolean);
...
async function initBrandData(brand) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      rows.forEach(r => {
        if(r&&!r.hata){
          urunler.push(r);
          appendRow(r, urunler.length-1);
        }
      });
    } catch(e) {
      console.warn(allLinks[i], e.message);
    }
  ...
  $('brand-title').innerHTML =
    `${activeBrand.charAt(0).toUpperCase()+activeBrand.slice(1)} `+
    `<span class="brand-meta">· ${urunler.length} ürün · Son Güncelleme: ${fmtFull(json.guncelleme)}</span>`;
...
async function refreshBrand(brand, data) {
  ...
  const linkSet = new Set(
    urunler.map(u=>productKey(u.urun_linki))
  );

  const newLinks = liveLinks.filter(
    l=>!linkSet.has(productKey(l))
  );
  ...
  for (let i = 0; i < total; i++) {
    ...
    const tr = domRows.find(r =>
      r.dataset.link===u.urun_linki &&
      (!u.varyant_adi ||
        r.querySelector('.urun-adi')?.textContent.includes(u.varyant_adi))
    );
      ...
      if (match&&!match.hata) {
        ...
        if (tr) {
          ...
          [
            ...
            cells[j].textContent=v;
            cells[j].className=`price ${out}`;
          });

          tr.classList.remove('updating');

          tr.querySelectorAll('.price').forEach(c=>{
            c.classList.add('flashed');
            setTimeout(()=>c.classList.remove('flashed'),1200);
          });
        }
      }
    } catch {}
  ...
  if (newLinks.length) {
    ...
    await kvSet(brand, json);
    setStatus('güncellendi — KV kaydedildi');
  }
...
function showNotice(brand, newLinks, json) {
  ...
  notice.innerHTML =
    `⚠ ${newLinks.length} yeni ürün <span class="new-badge">yükle</span>`;
    ...
    for (let i = 0; i < newLinks.length; i++) {
      ...
      try {
        ...
        rows.forEach(r=>{
          if(r&&!r.hata){
            json.urunler.push(r);
            appendRow(r, json.urunler.length-1);
          }
        });
      } catch {}
...
async function openBrand(slug, btn) {
  document.querySelectorAll('.brand-btn').forEach(
  ...
  if (!data) {
    ...
    $('brand-title').innerHTML =
      ...
      `<span class="brand-meta">· ${data.urunler.length} ürün · Son Güncelleme: ${fmtFull(data.guncelleme)}</span>`;

    setStatus('');

    const dupes = findDupes(data.urunler);

    $('btn-dedupe').classList.toggle(
      'hidden',
      dupes.length === 0
    );
  }
...
$('btn-refresh').onclick = async () => {
  if (!activeBrand||!brandData[activeBrand]) {
    setStatus('önce ilk çekimi yapın');
    return;
  }

  await refreshBrand(
    activeBrand,
    JSON.parse(JSON.stringify(brandData[activeBrand]))
  );
};
...
$('btn-dedupe').onclick = () => {
  ...
  if (!dupes.length) {
    setStatus('Tekrar yok');
    return;
  }

  const list = dupes.map(d =>
    `• [${d.idx+1}] ${d.u.sku||'-'} — ${d.u.urun_adi||''}`+
    `${d.u.varyant_adi?' · '+d.u.varyant_adi:''} `+
    `(ilk: sıra ${d.firstIdx+1})`
  ).join('\n');

  if (!confirm(
    `${dupes.length} tekrar bulundu — silinecekler:\n\n`+
    `${list}\n\nDevam edilsin mi?`
  )) return;
  ...
  const temiz = urunler.filter(
    (_, i) => !removeIdxs.has(i)
  );
  ...
  kvSet(activeBrand, json).then(() => {
    ...
    $('brand-title').innerHTML =
      `${activeBrand.charAt(0).toUpperCase()+activeBrand.slice(1)} `+
      `<span class="brand-meta">· ${temiz.length} ürün · Son Güncelleme: ${fmtFull(json.guncelleme)}</span>`;
...
(async () => {
  try {
    ...
    if(ts?.text) {
      loadTsoft(ts.text, true);

      const el=$('tsoft-status');

      if(el) {
        el.textContent =
          `✓ ${tsoftData.rows.length} ürün · ${ts.date||''}`;
      }
    }
  } catch {}
  ...
  try {
    ...
    if(ai?.text) {
      parseAide(ai.text, true);

      const el=$('aide-status');

      if(el) {
        el.textContent =
          `✓ ${aideData.size} kod · ${ai.date||''}`;
      }
    }
  } catch {}
})();

const AIDE_AMBARS = new Set([
  'sesci magaza',
  'sescibaba'
]);
...
function loadTsoft(text, skipKV) {
  ...
  if(activeBrand && brandData[activeBrand]) {
    applyMatching(brandData[activeBrand].urunler);
  }
}
...
function parseAide(text, skipKV) {
  ...
  for(const line of lines) {
    ...
    const stokKodu = (parts[2]||'')
      .trim()
      .toUpperCase();

    const stok =
      parseFloat((parts[4]||'0').replace(',','.')) || 0;

    if(!stokKodu) continue;

    aideData.set(
      stokKodu,
      (aideData.get(stokKodu)||0) + stok
    );
...
function matchTsoft(u, eanUnique) {
  ...
  if(u.sku) {
    const bySku =
      tsoftData.byWs.get(u.sku) ||
      tsoftData.bySup.get(u.sku);
  ...
  if(eanUnique) {
    ...
    for(const e of eans) {
      const r =
        tsoftData.byBarkod.get(e) ||
        tsoftData.byBarkod.get(e.replace(/^0+/,''));
...
function applyMatching(urunler) {
  ...
  const brand =
    activeBrand ? normBrand(activeBrand) : '';

  const rows = [
    ...tbody.querySelectorAll('tr[data-link]')
  ];

  const compelOnly = [];
  const tsoftMatched = new Set();
  ...
  urunler.forEach(u => {
    ...
    if(ean) {
      eanUsageCount[ean] =
        (eanUsageCount[ean] || 0) + 1;
    }
  });

  const sharedEans = new Set(
    Object.keys(eanUsageCount)
      .filter(e => eanUsageCount[e] > 1)
  );
  ...
  rows.forEach((tr, i) => {
    ...
    const tsUrunAdiCell =
      tr.querySelector('.ts-urun-adi');

    if(!tsCell||!tsStokCell||!aideCell) return;

    if(ts) {
      if(tsUrunAdiCell) {
        tsUrunAdiCell.textContent =
          ts['Ürün Adı']||'-';
      }

      const barkodCell =
        tr.querySelector('.ts-barkod');

      if(barkodCell) {
        barkodCell.textContent =
          ts['Barkod']||'-';
      }
      ...
      const tsUrl = tsSlug
        ? `https://www.sescibaba.com/${tsSlug}`
        : null;

      if(tsUrl) {
        tsCell.innerHTML =
          `<a href="${tsUrl}" target="_blank" style="color:inherit;text-decoration:none;cursor:pointer">${tsFiyatRaw||'-'}</a>`;
      } else {
      ...
      tsStokCell.onclick = () =>
        navigator.clipboard.writeText(tsSup);

      aideCell.textContent =
        aide!=null?aide:'-';
      ...
      tsStokCell.style.color =
        tsStokUyari ? '#ff6666' : '';

      tsStokCell.style.fontWeight =
        tsStokUyari ? 'bold' : '';

      tsStokCell.style.textDecoration = '';

      const farkCell =
        tr.querySelector('.fiyat-fark');

      if(farkCell) {
        const compelTL =
          tr.querySelectorAll('td')[10]?.textContent||'';
  ...
  const tsoftOnly = tsoftData
    ? tsoftData.rows.filter(r => {
        const aktif =
          (r['Aktif']||'')
            .toLowerCase()
            .trim();

        if(aktif==='false'||aktif==='0') {
          return false;
        }
        ...
        const sup =
          r['Tedarikçi Ürün Kodu']||'';

        return !tsoftMatched.has(ws) &&
          !tsoftMatched.has(sup);
      })
    : [];

  tbody.querySelectorAll('.ts-fiyat')
    .forEach(td => {
      ...
      if(a) {
        if(a.textContent &&
          a.textContent !== '-') {
          a.textContent =
            fmtPrice(a.textContent);
        }
      } else {
        if(td.textContent &&
          td.textContent !== '-') {
          td.textContent =
            fmtPrice(td.textContent);
        }
      }
    });
...
function renderUnmatched(compelOnly, tsoftOnly) {
  ...
  tsoftOnly.forEach((r, i) => {
    ...
    tr.innerHTML =
      `<td>${i+1}</td>`+
      `<td>${r['Web Servis Kodu']||'-'}</td>`+
      `<td>${r['Tedarikçi Ürün Kodu']||'-'}</td>`+
      `<td style="text-align:left">${r['Ürün Adı']||'-'}</td>`+
      `<td>${r['Stok']||'-'}</td>`+
      `<td>${fmtPrice(r['KDV Dahil Fiyat']||'')}</td>`+
      `<td>${r['Barkod']||'-'}</td>`;

    tt.appendChild(tr);
  });

  $('unmatched').classList.toggle(
    'hidden',
    !tsoftOnly.length
  );
}
...
function applyKutuHasarli() {
  const chk =
    document.getElementById('chk-kutu-hasarli');
...
$('data-modal-new').onclick = () => {
  ...
  if(dataModalMode === 'tsoft') {
    ...
    $('aide-modal').classList.remove('hidden');
  }
};
...
$('tsoft-btn').onclick = async () => {
  try {
    const ts = await kvGet('tsoft:data');
    openDataModal('tsoft', ts?.date||null);
  } catch {
    openDataModal('tsoft', null);
  }
};
...
$('aide-btn').onclick = async () => {
  try {
    const ai = await kvGet('aide:data');
    openDataModal('aide', ai?.date||null);
  } catch {
    openDataModal('aide', null);
  }
};
...
document
  .getElementById('chk-kutu-hasarli')
  ?.addEventListener(
    'change',
    applyKutuHasarli
  );
...
function updateThTop() {
  ...
  document.querySelectorAll('#main-table th')
    .forEach(th =>
      th.style.top = h + 'px'
    );
}
...
const thObserver =
  new ResizeObserver(updateThTop);
