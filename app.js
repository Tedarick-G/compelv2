...
function loadTsoft(text) {
  ...
  $('tsoft-status').textContent = `✓ ${rows.length} ürün`;
...
function parseAide(text) {
  ...
  for(const line of lines) {
    ...
    // Marka Model StokKodu Açıklama Stok Ambar Firma
...
function applyMatching(urunler) {
  ...
  rows.forEach((tr, i) => {
    ...
    // T-Soft Fiyat
    let tsCell = tr.querySelector('.ts-fiyat');
    if(!tsCell){ tsCell=document.createElement('td'); tsCell.className='ts-fiyat ts-col'; tr.appendChild(tsCell); }
    let tsStokCell = tr.querySelector('.ts-stok');
    if(!tsStokCell){ tsStokCell=document.createElement('td'); tsStokCell.className='ts-stok ts-col'; tr.appendChild(tsStokCell); }
    let aideCell = tr.querySelector('.aide-stok');
    if(!aideCell){ aideCell=document.createElement('td'); aideCell.className='aide-stok ts-col'; tr.appendChild(aideCell); }

    if(ts) {
      tsCell.textContent = ts['KDV Dahil Fiyat'] || '-';
      ...
      tsoftMatched.add(ts['Web Servis Kodu'] || ts['Tedarikçi Ürün Kodu']);
    } else {
...
function renderUnmatched(compelOnly, tsoftOnly) {
  const tc = $('tbody-compel-only'), tt = $('tbody-tsoft-only');
  tc.innerHTML = '';
  compelOnly.forEach(u => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${u.sku||'-'}</td><td>${u.urun_adi||'-'}${u.varyant_adi?` · ${u.varyant_adi}`:''}</td><td>${u.ean||'-'}</td><td>${u.stok??'-'}</td><td>${u.guncel_fiyat||'-'}</td>`;
    tc.appendChild(tr);
  });
  tt.innerHTML = '';
  tsoftOnly.forEach(r => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${r['Tedarikçi Ürün Kodu']||'-'}</td><td>${r['Ürün Adı']||'-'}</td><td>${r['Barkod']||'-'}</td><td>${r['Stok']||'-'}</td><td>${r['KDV Dahil Fiyat']||'-'}</td><td>${r['Aktif']||'-'}</td>`;
    tt.appendChild(tr);
  });
  $('unmatched').classList.toggle('hidden', !compelOnly.length && !tsoftOnly.length);
}
...
$('aide-btn').onclick = () => $('aide-modal').classList.remove('hidden');
