...
function parseCompelFullRows(items){
  const rows=(items||[]).map(x=>({
    Marka:T(x.brand),
    "Ürün Adı":T(x.title),
    "Ürün Kodu":T(x.productCode),
    Stok:T(x.stock),
    EAN:T(x.ean),
    Link:T(x.url)
  }));
  return {
    hdr:["Marka","Ürün Adı","Ürün Kodu","Stok","EAN","Link"],
...
function renderList(listCode,title,hdr,rows,target){
  ...
  const body=rows.map(r=>`<tr>${hdr.map(h=>{
    const v=r[h]??"";
    return h.toLocaleLowerCase(TR)==="link"&&T(v)
      ? `<td><a href="${v}" target="_blank">Aç</a></td>`
      : `<td>${String(v??"")}</td>`;
  }).join("")}</tr>`).join("");
  target.innerHTML=
...
async function list(){
  try{
    ...
    const l1=parseCompelFullRows(compelRes.items||[]);
