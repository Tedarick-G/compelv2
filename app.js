...
(async () => {
  try {
    ...
    if(ts?.text) { loadTsoft(ts.text, true); $('tsoft-status').textContent = `✓ ${tsoftData.rows.length} ürün · ${ts.date||''}`; }
  } catch {}
  try {
    const ai = await kvGet('aide:data');
    if(ai?.text) { parseAide(ai.text, true); $('aide-status').textContent = `✓ ${aideData.size} kod · ${ai.date||''}`; }
  } catch {}
})();
...
function loadTsoft(text, skipKV) {
  ...
  $('tsoft-status').textContent = `✓ ${rows.length} ürün · ${date}`;
  if(!skipKV) {
...
function parseAide(text, skipKV) {
  ...
  const date = fmtFull(new Date().toISOString());
  $('aide-status').textContent = `✓ ${aideData.size} kod · ${date}`;
  if(!skipKV) {
...
function renderUnmatched(compelOnly, tsoftOnly) {
  ...
  tsoftOnly.forEach(r => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${r['Tedarikçi Ürün Kodu']||'-'}</td><td class="col-urun">${r['Ürün Adı']||'-'}</td><td>${r['Stok']||'-'}</td><td>${r['KDV Dahil Fiyat']||'-'}</td>`;
    tt.appendChild(tr);
  });
...
$('tsoft-btn').onclick = async () => {
  try {
    const ts = await kvGet('tsoft:data');
    if(ts?.text && ts?.date) {
      if(confirm(`${ts.date} Tarihli veri yüklensin mi?`)) {
        loadTsoft(ts.text, true);
        $('tsoft-status').textContent = `✓ ${tsoftData.rows.length} ürün · ${ts.date}`;
      }
    } else { alert('Kayıtlı T-Soft verisi yok. "Yeni CSV" ile yükleyin.'); }
...
$('aide-btn').onclick = async () => {
  try {
    const ai = await kvGet('aide:data');
    if(ai?.text && ai?.date) {
      if(confirm(`${ai.date} Tarihli veri yüklensin mi?`)) {
        parseAide(ai.text, true);
        $('aide-status').textContent = `✓ ${aideData.size} kod · ${ai.date}`;
      }
    } else { alert('Kayıtlı Aide verisi yok. "Yeni Yapıştır" ile yükleyin.'); }
  } catch(e) { alert('KV hatası: ' + e.message); }
};

$('aide-new-btn').onclick = () => $('aide-modal').classList.remove('hidden');
