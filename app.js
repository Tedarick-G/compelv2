...
const setDate = iso => { jsonDate.textContent = iso ? fmtDate(iso) : ''; };
...
async function initBrandData(brand) {
  ...
  setStatus(`${urunler.length} ürün tamamlandı · Son güncelleme: ${fmtFull(json.guncelleme)}`);
  return json;
}
...
async function openBrand(slug, btn) {
  ...
  notice.classList.add('hidden');
  $('brand-title').textContent = slug.charAt(0).toUpperCase()+slug.slice(1);
  ...
  if (!data) {
    ...
    setStatus(`${data.urunler.length} ürün yüklendi · Son güncelleme: ${fmtFull(data.guncelleme)}`);
  }
