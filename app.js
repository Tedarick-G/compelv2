...
const brandList = $('brand-list'), productSection = $('product-section'),
  ...
  notice = $('notice'), status = $('status'), jsonDate = $('json-date');

function setStatus(msg, dur=3000) {
  status.textContent = msg; status.classList.remove('hidden');
...
async function initBrandData(brand) {
  ...
  setStatus(`${urunler.length} ürün indirildi. data/compel/ klasörüne koy.`);
  return json;
}
...
async function openBrand(brand) {
  ...
  if (!data) {
    ...
    setStatus(`${data.urunler.length} ürün yüklendi`, 3000);
  }
}
