...
function sortTable(col) {
  const rows = [...tbody.querySelectorAll('tr[data-link]')];
  if(!origOrder.length) origOrder = rows.map(r => r);

  if(col === 'idx') {
    // Sıfırla
    origOrder.forEach(r => tbody.appendChild(r));
    sortState = { col: null, asc: true };
    document.querySelectorAll('.sort-btn').forEach(b => b.textContent = b.textContent.replace(/[↑↓]/,'↕'));
    return;
  }
  ...
  const getVal = (tr) => {
    if(col === 'stok') return parseNum(tr.querySelector('td:nth-child(7)')?.textContent);
    if(col === 'tl')   return parseNum(tr.querySelector('td:nth-child(10)')?.textContent);
    return 0;
  };
  ...
  document.querySelectorAll('.sort-btn').forEach(b => {
    const bc = b.dataset.sort;
    b.textContent = b.textContent.replace(/[↑↓↕]/g, bc===col?(asc?'↑':'↓'):'↕');
  });
}
