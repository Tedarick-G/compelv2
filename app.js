...
function setLastUpdate(iso){
  ...
  document.getElementById('last-update').textContent=`Son güncelleme: ${iso.slice(0,10)} ${DAYS[d.getDay()]} | ${iso.slice(11,16)}`;
}
...
async function initBrandData(brand) {
  ...
  setDate(json.guncelleme); setLastUpdate(json.guncelleme);
...
async function refreshBrand(brand, data) {
  ...
  else { await kvSet(brand, json); setLastUpdate(json.guncelleme); setStatus('güncellendi — KV kaydedildi'); }
...
async function openBrand(slug, btn) {
  ...
  if (!data) {
    ...
    renderTable(data.urunler); setDate(data.guncelleme); setLastUpdate(data.guncelleme);
