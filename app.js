...
function normHeader(h) {
  return T(h).toLocaleUpperCase(TR).replace(/\s+/g, " ");
}
...
const ALIAS = new Map([
  ...
  ["WARMAUDIO", "WARM AUDIO"], ["BEYER", "BEYERDYNAMIC"], ["BEYERDYNAMIC", "BEYERDYNAMIC"],
  ["ALLENHEATH", "ALLEN HEATH"], ["MARANTZPROFESSIONAL", "MARANTZ"], ["RUPERTNEVEDESIGNS", "RUPERT NEVE"],
]);
...
async function loadMeta() {
  try { state.meta = await api("/api/daily/meta"); }
  catch { state.meta = null; }
  paintDailyBtns();
}

function pickHM(obj) {
  return T(obj?.hm || obj?.HM || obj?.time || obj?.saat || "");
}
...
function dailyPick(kind) {
  ...
  if (t?.exists) {
    return {
      ...
      label: `${kind === "tsoft" ? "T-Soft" : "Aide"} ${ymdToDmy(state.meta.today.ymd)} Tarihli Veri`,
      hm: pickHM(t),
    };
  }
  ...
  if (y?.exists) {
    return {
      ...
      label: `${kind === "tsoft" ? "T-Soft" : "Aide"} ${state.meta.yesterday.dmy} Tarihli Veri`,
      hm: "",
    };
  }
  ...
  for (const kind of ["tsoft", "aide"]) {
    ...
    btn.textContent = pick
      ? state.selDaily[kind] === pick.ymd
        ? `${kind === "tsoft" ? "T-Soft" : "Aide"} Seçildi`
        : pick.hm
          ? `${kind === "tsoft" ? "T-Soft" : "Aide"} ${pick.hm}`
          : pick.label
      : `${kind === "tsoft" ? "T-Soft" : "Aide"} Veri Yok`;
  }
}
...
function ensureSaveCred() {
  ...
  if (!T(admin) || !T(read)) throw new Error("Şifre girilmedi");
...
document.addEventListener("click", (e) => {
  if (!$("#brandWrap").contains(e.target)) $("#brandWrap").classList.remove("open");
});
...
function depotFromNoisyPaste(text) {
  ...
  const skip = (s) =>
    !s || /^(Tümü|Sesçibaba Logo|Şirketler|Siparişler|Onay Bekleyen|Sipariş Listesi|İade Listesi|Sesçibaba Stokları|Stok Listesi|Ara|Previous|Next|Showing\b.*|\d+)$/.test(s);
...
function selectedBrandNormSet() {
  return new Set(state.brands.filter((b) => state.selBrands.has(b.id)).map((b) => normBrand(b.name)));
}

function filterBySelectedBrand(rows, getBrand) {
  const sel = selectedBrandNormSet();
  if (!sel.size) return rows;
  return rows.filter((r) => sel.has(normBrand(getBrand(r))));
}
...
function parseTsoftRawToFullRows(raw) {
  ...
  return {
    hdr: p.hdr,
    rows: filterBySelectedBrand(rows, (r) => r[brandCol] || ""),
  };
}
...
function parseAideRawToFullRows(raw) {
  ...
  return {
    ...
    rows: filterBySelectedBrand(noisy, (r) => r["Marka"] || ""),
  };
}
...
function parseCompelFullRows(items) {
  const rows = flattenCompelItems(items);
  return {
...
function renderList(code, title, hdr, rows, target) {
  ...
  const cell = (h, v) => {
    const x = String(v ?? "");
    return /link|görsel/i.test(h) && T(x)
      ? `<td><a href="${x}" target="_blank">Aç</a></td>`
      : `<td>${x}</td>`;
  };

  target.innerHTML =
    `<h3>${title} <small>${code}</small></h3>` +
    `<div class="meta">Satır: ${rows.length} | Sütun: ${hdr.length}</div>` +
    `<table><thead><tr>${head}</tr></thead><tbody>${
      rows.map((r) => `<tr>${hdr.map((h) => cell(h, r[h])).join("")}</tr>`).join("")
    }</tbody></table>`;
}
  ...
  for (;;) {
    ...
    while ((i = buf.indexOf("\n")) >= 0) {
      ...
      if (m.type === "page") {
        ...
        addLog(m.msg || JSON.stringify(m));
      } else if (m.type === "brandDone") {
        addLog(`${m.brand} tamam`);
      }
    }
  }
